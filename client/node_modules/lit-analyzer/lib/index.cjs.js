...
function makeConfig(userOptions) {
    ...
    return {
        ...
        format: {
            disable: userOptions.format != null ? userOptions.format.disable : false
        },
        // Template tags
        htmlTemplateTags: userOptions.htmlTemplateTags || ["html", "raw"],
        cssTemplateTags: userOptions.cssTemplateTags || ["css"],
        // Global additions
        globalTags: userOptions.globalTags || userOptions.externalHtmlTagNames || [],
...
function documentationForHtmlTag(htmlTag, options) {
    ...
    if (htmlTag.events.length > 0) {
        ...
        desc += "\n\n" + descriptionList("Events", items, function (event) { return "" + descriptionHeader("@event " + event.name, 0, options) + (event.description ? " - " + event.description : ""); }, options);
    }
...
var VirtualAstDocument = /** @class */ (function () {
    function VirtualAstDocument(astNodeOrParts, location, fileName) {
        ...
        if (Array.isArray(astNodeOrParts)) {
            this.parts = astNodeOrParts.map(function (p, i) { return (typeof p === "string" ? "" + (i !== 0 ? "}" : "") + p + (i !== astNodeOrParts.length - 1 ? "${" : "") : p); });
    ...
    VirtualAstDocument.prototype.getPartsAtOffsetRange = function (range) {
        ...
        this.parts.forEach(function (part, i) {
            ...
            if (typeof part === "string") {
                ...
                if ((range.start < literalPartRange.start && range.end > literalPartRange.end) || (intersects(range.start + 1, literalPartRange) || intersects(range.end - 1, literalPartRange))) {
...
var HtmlDocument = /** @class */ (function (_super) {
    ...
    HtmlDocument.prototype.htmlAttrAssignmentAtOffset = function (offset) {
        return this.findAttr(function (attr) { return (attr.assignment != null && attr.assignment.location != null ? intersects(offset, attr.assignment.location) : false); });
    };
...
(function (LitHtmlDiagnosticKind) {
    ...
    LitHtmlDiagnosticKind["PRIMITIVE_NOT_ASSIGNABLE_TO_COMPLEX"] = "PRIMITIVE_NOT_ASSIGNABLE_TO_COMPLEX";
    LitHtmlDiagnosticKind["COMPLEX_NOT_ASSIGNABLE_TO_PRIMITIVE"] = "COMPLEX_NOT_ASSIGNABLE_TO_PRIMITIVE";
...
function iterableMap(iterable, map) {
    var e_3, _a, iterable_2, iterable_2_1, item, e_3_1;
...
function iterableFilter(iterable, filter) {
    var e_4, _a, iterable_3, iterable_3_1, item, e_4_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 5, 6, 7]);
    ...
    switch (htmlReport.kind) {
        case exports.LitHtmlDiagnosticKind.UNKNOWN_TARGET: {
            ...
            if (htmlReport.suggestedTarget != null) {
                var newText_1 = "" + litAttributeModifierForTarget(htmlReport.suggestedTarget) + htmlReport.suggestedTarget.name;
                fixes.push({
                    kind: exports.CodeFixKind.RENAME,
                    message: "Change " + (htmlReport.htmlAttr.kind === HtmlNodeAttrKind.PROPERTY ? "property" : "attribute") + " to '" + newText_1 + "'",
                    htmlReport: htmlReport,
                    actions: [
                        {
                            kind: exports.CodeActionKind.DOCUMENT_TEXT_CHANGE,
...
function completionsForHtmlAttrs(htmlNode, location, _a) {
    ...
    // Code completions for "?[...]";
        ...
        var alreadyUsedAttrNames_1 = htmlNode.attributes.filter(function (a) { return a.modifier === LIT_HTML_BOOLEAN_ATTRIBUTE_MODIFIER || a.modifier == null; }).map(function (a) { return a.name; });
...
function validateHtmlAttr(htmlAttr, request) {
    ...
    if (htmlAttrTarget == null) {
        ...
        switch (htmlAttr.kind) {
            case HtmlNodeAttrKind.ATTRIBUTE:
            case HtmlNodeAttrKind.BOOLEAN_ATTRIBUTE:
                if (config.skipUnknownAttributes)
                    return [];
                break;
...
function validateHtmlAttrAssignment(htmlAttr, request) {
    ...
    var typeB = tsSimpleType.isSimpleType(typeBInferred) ? typeBInferred : tsSimpleType.toSimpleType(typeBInferred, checker);
...
function validateHtmlAttrAssignmentRules(htmlAttr, typeB, _a) {
    var document = _a.document, htmlStore = _a.htmlStore, config = _a.config, program = _a.program;
    var assignment = htmlAttr.assignment;
    if (assignment == null)
        return undefined;
    switch (htmlAttr.kind) {
        case HtmlNodeAttrKind.EVENT_LISTENER:
            ...
            if (!tsSimpleType.isAssignableToSimpleTypeKind(typeB, [tsSimpleType.SimpleTypeKind.FUNCTION, tsSimpleType.SimpleTypeKind.METHOD, tsSimpleType.SimpleTypeKind.UNKNOWN], {
                matchAny: true,
                op: "or"
            })) {
                return [
            ...
            if (parentHtmlTag != null && parentHtmlTag.slots.length > 0) {
                ...
                if (matchingSlot == null) {
                    ...
                    var message = validSlotNames.length === 1 && validSlotNames[0].length === 0
                        ...
                        : "Invalid slot name '" + slotName_1 + "'. Valid slot names for <" + parentHtmlTag.tagName + "> are: " + validSlotNames.map(function (n) { return "'" + n + "'"; }).join(" | ");
                    return [
                        {
...
function validateHtmlAttrAssignmentTypes(htmlAttr, _a, _b) {
    var _c;
    var typeA = _a.typeA, typeB = _a.typeB;
    var program = _b.program, document = _b.document;
    var assignment = htmlAttr.assignment;
    if (assignment == null)
        return undefined;
    switch (htmlAttr.modifier) {
        ...
        default:
                ...
                var message = (function () {
                    if (assignment != null) {
                        if (assignment.kind === HtmlNodeAttrAssignmentKind.BOOLEAN) {
                            return "You are assigning a boolean to a non-primitive type '" + tsSimpleType.toTypeString(typeA) + "'. Use '.' binding instead?";
                        }
                        else if (assignment.kind === HtmlNodeAttrAssignmentKind.STRING && assignment.value.length > 0) {
                            return "You are assigning the string '" + tsSimpleType.toTypeString(typeB) + "' to a non-primitive type '" + tsSimpleType.toTypeString(typeA) + "'. Use '.' binding instead?";
                        }
                    }
                    return "You are assigning the primitive '" + tsSimpleType.toTypeString(typeB) + "' to a non-primitive type '" + tsSimpleType.toTypeString(typeA) + "'. Use '.' binding instead?";
                })();
            ...
            else if (!tsSimpleType.isAssignableToPrimitiveType(typeB)) {
                return [
                    {
                        kind: exports.LitHtmlDiagnosticKind.COMPLEX_NOT_ASSIGNABLE_TO_PRIMITIVE,
                        severity: "error",
                        message: "You are assigning a non-primitive type '" + tsSimpleType.toTypeString(typeB) + "' to a primitive type '" + tsSimpleType.toTypeString(typeA) + "'",
                        location: __assign({ document: document }, htmlAttr.location.name),
                        htmlAttr: htmlAttr,
                        typeA: typeA,
                        typeB: typeB
            ...
            var typeAIsAssignableTo_1 = (_c = {},
                ...
                _c[tsSimpleType.SimpleTypeKind.OBJECT] = lazy(function () { return tsSimpleType.isAssignableToSimpleTypeKind(typeA, [tsSimpleType.SimpleTypeKind.OBJECT, tsSimpleType.SimpleTypeKind.INTERFACE, tsSimpleType.SimpleTypeKind.CLASS], { op: "or" }); }),
                _c);
            var typeAIsAssignableToMultiple = lazy(function () { return Object.values(typeAIsAssignableTo_1).filter(function (assignable) { return assignable(); }).length > 1; });
            ...
            if (typeB.kind === tsSimpleType.SimpleTypeKind.STRING_LITERAL) {
                ...
                if (typeB.value.length === 0 && typeAIsAssignableTo_1[tsSimpleType.SimpleTypeKind.BOOLEAN]()) {
                    return [];
                }
                ...
                else if (typeAIsAssignableTo_1[tsSimpleType.SimpleTypeKind.NUMBER]()) {
            ...
            // Tagged template attribute 'expression' assignments like 'max="${this.max}"'
                ...
                if (typeAIsAssignableTo_1[tsSimpleType.SimpleTypeKind.BOOLEAN]() && !typeAIsAssignableToMultiple()) {
                    return [
                        {
                ...
                else if (tsSimpleType.isAssignableToSimpleTypeKind(typeB, [tsSimpleType.SimpleTypeKind.NUMBER, tsSimpleType.SimpleTypeKind.NUMBER_LITERAL], { op: "or" }) && typeAIsAssignableTo_1[tsSimpleType.SimpleTypeKind.STRING]()) {
                    return [];
                }
                // Take into account string === boolean expressions: 'aria-expanded="${this.open}"'
                else if (tsSimpleType.isAssignableToSimpleTypeKind(typeB, [tsSimpleType.SimpleTypeKind.BOOLEAN, tsSimpleType.SimpleTypeKind.BOOLEAN_LITERAL], { op: "or" }) && tsSimpleType.isAssignableToType(typeA, STRINGIFIED_BOOLEAN_TYPE, program)) {
                    return [];
                }
            }
    }
    if (!tsSimpleType.isAssignableToType(typeA, typeB, program)) {
        ...
        if (assignment.kind === HtmlNodeAttrAssignmentKind.EXPRESSION && htmlAttr.kind === HtmlNodeAttrKind.ATTRIBUTE) {
            if (tsSimpleType.isAssignableToSimpleTypeKind(typeB, tsSimpleType.SimpleTypeKind.UNDEFINED)) {
                var typeBWithoutUndefined = removeUndefinedFromType(typeB);
                if (tsSimpleType.isAssignableToType(typeA, typeBWithoutUndefined, program)) {
                    return [
                        {
                            kind: exports.LitHtmlDiagnosticKind.INVALID_ATTRIBUTE_EXPRESSION_TYPE_UNDEFINED,
                            message: "Type '" + tsSimpleType.toTypeString(typeB) + "' is not assignable to '" + tsSimpleType.toTypeString(typeA) + "'. Fix it using 'ifDefined'?",
                            severity: "error",
                            location: __assign({ document: document }, htmlAttr.location.name),
                            htmlAttr: htmlAttr,
                            typeA: typeA,
...
function validateHtmlAttrDirectiveAssignment(htmlAttr, _a, request) {
    ...
    if (tsSimpleType.isAssignableToSimpleTypeKind(typeB, tsSimpleType.SimpleTypeKind.FUNCTION)) {
    ...
    // Make sure that "classMap" and "styleMap" directives are not used in mixed bindings.
        ...
        var directivesUnavailableInMixed = assignment.values
            .filter(function (value) { return typeof value !== "string" && ts.isCallExpression(value); })
            .filter(function (exp) { return ["classMap", "styleMap"].includes(exp.expression.getText()); })
            .map(function (exp) { return ({
            type: tsSimpleType.toSimpleType(checker.getTypeAtLocation(exp), checker),
            name: exp.expression.getText()
        }); })
            .filter(function (directive) { return isLitDirective(directive.type); });
        if (directivesUnavailableInMixed.length > 0) {
            var directiveName = directivesUnavailableInMixed[0].name;
            return [
                {
                    kind: exports.LitHtmlDiagnosticKind.DIRECTIVE_NOT_ALLOWED_HERE,
    ...
    var collection = (function () {
        ...
        try {
            for (var _b = __values(Array.isArray(config.customHtmlData) ? config.customHtmlData : [config.customHtmlData]), _c = _b.next(); !_c.done; _c = _b.next()) {
                ...
                try {
                    var data = typeof customHtmlData === "string" && fs.existsSync(customHtmlData) ? JSON.parse(fs.readFileSync(customHtmlData, "utf8").toString()) : customHtmlData;
...
function getAssignmentLocation(p5Node, p5Attr, htmlAttr, context) {
    var htmlAttrLocation = (p5Node.sourceCodeLocation.startTag.attrs || {})[p5Attr.name];
    ...
    var firstQuote = htmlAfterName.indexOf('"');
    var lastQuote = htmlAfterName.lastIndexOf('"');
...
function makeHtmlAttrLocation(p5Node, p5Attr, context) {
    ...
    var htmlAttrLocation = (p5Node.sourceCodeLocation.startTag.attrs || {})[sourceCodeLocationName];
...
function isSelfClosed(p5Node, context) {
    ...
    var isSelfClosed = p5Node.sourceCodeLocation.startTag.endOffset === p5Node.sourceCodeLocation.endOffset;
...
function makeHtmlNodeLocation(p5Node, context) {
    var loc = p5Node.sourceCodeLocation;
    return {
...
var HtmlDataSourceMerged = /** @class */ (function () {
    ...
    HtmlDataSourceMerged.prototype.iterateAllPropertiesForNode = function (tagName) {
        var e_4, _a, htmlTag, extensions, extensions_1, extensions_1_1, extTag, e_4_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
    ...
    HtmlDataSourceMerged.prototype.iterateAllEventsForNode = function (tagName) {
        var e_5, _a, e_6, _b, e_7, _c, e_8, _d, e_9, _e, htmlTag, extensions, extensions_2, extensions_2_1, extTag, e_5_1, _f, _g, tag, e_6_1, eventNameSet, _h, _j, tag, _k, _l, evt, e_8_1, e_7_1, _m, _o, evt, e_9_1;
        return __generator(this, function (_p) {
            switch (_p.label) {
                ...
                case 9:
                    try {
                        if (extensions_2_1 && !extensions_2_1.done && (_a = extensions_2.return)) _a.call(extensions_2);
                    }
                ...
                case 11:
                    ...
                    _f = __values(this.globalTags.values()), _g = _f.next();
                    _p.label = 12;
                case 12:
                ...
                case 21:
                    ...
                    _h = __values(this.globalTags.values()), _j = _h.next();
                    _p.label = 22;
                case 22:
                ...
                case 23:
                    ...
                    _k = __values(tag.events), _l = _k.next();
                    _p.label = 24;
                case 24:
                    if (!!_l.done) return [3 /*break*/, 27];
                    evt = _l.value;
                ...
                case 34:
                    ...
                    _m = __values(this.iterateGlobalEvents()), _o = _m.next();
                    _p.label = 35;
                case 35:
                    if (!!_o.done) return [3 /*break*/, 38];
                    evt = _o.value;
    ...
    HtmlDataSourceMerged.prototype.iterateAllAttributesForNode = function (tagName) {
        var e_10, _a, htmlTag, extensions, extensions_3, extensions_3_1, extTag, e_10_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    htmlTag = this.getHtmlTag(tagName);
    ...
    HtmlDataSourceMerged.prototype.iterateAllSlotsForNode = function (tagName) {
        var e_11, _a, htmlTag, extensions, extensions_4, extensions_4_1, extTag, e_11_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    htmlTag = this.getHtmlTag(tagName);
...
function changedSourceFileIterator() {
    ...
    var iterator = function (sourceFiles) {
        var e_1, _a, sourceFiles_1, sourceFiles_1_1, sourceFile, e_1_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 5, 6, 7]);
...
var DefaultLitAnalyzerContext = /** @class */ (function () {
    ...
    DefaultLitAnalyzerContext.prototype.findDependenciesInFile = function (file) {
        if (this.config.skipMissingImports)
            return;
 ...
 * @param context
 */
function analyzeGlobs(globs, context) {
    if (context === void 0) { context = {}; }
    return __awaiter(this, void 0, void 0, function () {
        var e_1, _a, filePaths, _b, program, files, diagnostics, files_1, files_1_1, file;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, expandGlobs(globs)];
                case 1:
                    filePaths = _c.sent();
                    // Callbacks
                    ...
                    _b = compileTypescript(filePaths), program = _b.program, files = _b.files, diagnostics = _b.diagnostics;
                    ...
                    try {
                        ...
                        for (files_1 = __values(files), files_1_1 = files_1.next(); !files_1_1.done; files_1_1 = files_1.next()) {
                            ...
                            if (context.analyzeSourceFile != null)
                                context.analyzeSourceFile(file, { program: program });
                        }
                    }
                    catch (e_1_1) { e_1 = { error: e_1_1 }; }
                    finally {
...
function cli() {
    return __awaiter(this, void 0, void 0, function () {
        var args, glob, program, context, analyzer;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, analyzeGlobs([glob], {
                            analyzeSourceFile: function (file, options) {
                                console.log("Analyzing file: " + file.fileName);
                                program = options.program;
                                var result = analyzer.getDiagnosticsInFile(file);
                                result.map(function (r) {
                                    var textSpan = translateRange(r.location);
                                    var text = generateSourceFileErrorText(file, textSpan, r.message);
                                    console.log(text);
                                });
                            }
                        })];
...
function generateSourceFileErrorText(file, textSpan, message) {
    ...
    var lineText = file.getText().substring(linePositionRange.start, linePositionRange.end);
...
exports.cli = cli;
exports.generateSourceFileErrorText = generateSourceFileErrorText;
