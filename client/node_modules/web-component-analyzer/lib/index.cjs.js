...
function resolveNodeValue(node, context) {
    ...
    if (ts.isStringLiteralLike(node)) {
        ...
        try {
            ...
            var json = node.getText().replace(regex, function (fullGroup, groupMatch) { return "\"" + groupMatch + "\":"; });
 ...
 * @param context
 */
function parseDeclarationCSSParts(node, context) {
    var ts = context.ts;
    if (ts.isInterfaceDeclaration(node) || ts.isClassDeclaration(node)) {
        return parseJsDocForNode(node, ["csspart"], function (tagNode, parsed) {
            if (parsed.name != null) {
                return {
                    name: parsed.name,
                    jsDoc: parsed.comment != null ? { comment: parsed.comment } : undefined
...
var JsDocFlavor = /** @class */ (function () {
    ...
    JsDocFlavor.prototype.parseDeclarationCSSParts = function (node, context) {
...
function parseComponentDeclaration(declarationNode, flavors, context) {
    ...
    visitComponentDeclaration(declarationNode, flavors, __assign(__assign({}, context), { declarationNode: declarationNode, features: {
        ...
        emitCSSParts: function (newCSSParts) {
            cssParts.push.apply(cssParts, __spread(newCSSParts));
        },
    ...
    var mergedEvents = mergeEvents(events);
    var mergedCSSProps = mergeCSSProps(cssProps);
    var mergedCSSParts = mergeCSSParts(cssParts);
...
function visitComponentDeclaration(node, flavors, context) {
    ...
    // Emit css parts
    var cssPartsResult = executeFirstFlavor(flavors, "parseDeclarationCSSParts", node, context);
    if (cssPartsResult != null) {
        context.emitCSSParts(cssPartsResult.result);
        if (!cssPartsResult.shouldContinue)
            return;
    }
    // Emit slots
...
function definitionToHtmlDataTag(definition, checker) {
    ...
    var cssProperties = definition.declaration.cssProperties
        ...
        .filter(function (val) { return val != null; });
    var cssParts = definition.declaration.cssParts
...
function cssPartSection(cssPart, config) {
    ...
    rows.push.apply(rows, __spread(cssPart.map(function (part) { return [(part.name && markdownHighlight(part.name)) || "", (part.jsDoc && part.jsDoc.comment) || ""]; })));
