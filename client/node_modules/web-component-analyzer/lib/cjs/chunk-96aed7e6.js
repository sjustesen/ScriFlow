...
function resolveNodeValue(node, context) {
    ...
    // Resolve values of variables.
        ...
        return resolveNodeValue(declaration[0], exports.__assign(exports.__assign({}, context), { depth: depth }));
    }
    // Fallthrough
    //  - "my-value" as string
    //  - <any>"my-value"
            ...
            try {
                for (var declarations_1 = __values(declarations), declarations_1_1 = declarations_1.next(); !declarations_1_1.done; declarations_1_1 = declarations_1.next()) {
                    ...
                    if (returnNode != null && returnNode.expression != null && returnNode.expression !== node) {
                        resolveHeritageClause(heritage, returnNode.expression, exports.__assign(exports.__assign({}, context), { emitHeritageClause: function (clause) {
            ...
            return parseJsDocForNode(node, ["event", "fires", "emits"], function (tagNode, _a) {
                ...
                if (name != null && name.length > 0 && tagNode != null) {
                    return {
                        name: name,
                        jsDoc: description != null ? { description: description } : undefined,
                        type: lazy(function () { return (type && parseJsDocTypeExpression(type)) || { kind: tsSimpleType.SimpleTypeKind.ANY }; }),
                        typeHint: type,
                        node: tagNode
                    };
                }
...
function applyJsDocType(feature, jsDoc) {
    ...
    if (typeTag != null && feature.typeHint == null) {
        ...
        if (parsed_1.type != null && parsed_1.type.length > 0) {
            return exports.__assign(exports.__assign({}, feature), { typeHint: parsed_1.type, type: (_b = feature.type, (_b !== null && _b !== void 0 ? _b : lazy(function () { return parseJsDocTypeExpression(parsed_1.type || ""); }))) });
        }
    }
    return feature;
}
...
function mergeMethods(methods) {
    return mergeNamedEntries(methods, function (method) { return method.name; }, function (left, right) { return (exports.__assign(exports.__assign({}, left), { jsDoc: mergeJsDocIntoJsDoc(left.jsDoc, right.jsDoc) })); });
}
function mergeEvents(events) {
    return mergeNamedEntries(events, function (event) { return event.name; }, function (left, right) { return (exports.__assign(exports.__assign({}, left), { jsDoc: mergeJsDocIntoJsDoc(left.jsDoc, right.jsDoc) })); });
}
...
function visitAndExpandInheritClause(inheritClause, context) {
    ...
    return exports.__assign(exports.__assign({}, inheritClause), { horizontalInherits: (_a = inheritClause.horizontalInherits) === null || _a === void 0 ? void 0 : _a.map(function (arg) { return visitAndExpandInheritClause(arg, context); }), resolved: resolved });
}
...
function discoverDefinitions$4(sourceFile, context, analyzeDeclaration) {
    ...
    return Array.from(definitionResults.entries()).map(function (_a) {
        ...
        return exports.__assign(exports.__assign({}, definition), { declaration: lazy(function () { return analyzeDeclaration(Array.from(declarationSet)); }) });
    });
...
function analyzeSourceFile(sourceFile, options) {
    ...
    var context = {
        ...
        config: exports.__assign({ analyzeLibDom: false, excludedDeclarationNames: [], features: ALL_COMPONENT_FEATURES }, (options.config || {}))
    };
...
function stripTypescriptValues(input, checker) {
    ...
    if (input == null) {
        ...
        return "{SIMPLETYPE:" + tsSimpleType.toTypeString(input) + "}";
    }
    else if (Array.isArray(input)) {
        return input.map(function (i) { return stripTypescriptValues(i, checker); });
    }
    else if (input instanceof Object) {
        var obj = exports.__assign({}, input);
        try {
...
function getTypeHintFromType(type, checker) {
    if (type == null)
        return undefined;
    var typeHint = isTypescriptType$1(type) ? checker.typeToString(type) : tsSimpleType.toTypeString(type);
    if (typeHint === "any")
        return undefined;
    return typeHint;
}
